...
import TimeJoke from './TimeJoke';

const Result = ({ initialTime, remainTime, result }) => {
    ...
    useEffect(() => {
        ...
        result.results.forEach((submission) => {
            if (submission.correct_answer === submission.choice) {
                // console.log("correct")
                setCorrectMarked((correctMarked) => [
                    ...correctMarked,
                    submission,
                ]);
                setScore(
                    (score) => score + correctMarks(submission.difficulty),
                );
            } else {
                // console.log("incorrectcorrect")
                setIncorrectMarked((incorrectMarked) => [
                    ...incorrectMarked,
                    submission,
                ]);
    ...
    return (
        <>
            ...
            <h1 className="text-7xl">
                Your Score is:{' '}
                <span className="text-[#4D6B73] bg-transparent">{score}</span>
            </h1>
            <h2 className="text-2xl inline-block m-3">
                <span className="text-[#6BAA75]">
                    <b>{correctMarked.length}</b>
                </span>{' '}
                questions were scored correct, while{' '}
                <span className="text-[#D64045]">
                    <b>{incorrectMarked.length}</b>
                </span>{' '}
            ...
            <h2 className="m-2 text-3xl text-[#6BAA75] underline">
                Corsssrectly Marked Questions
            </h2>

            <div className="correct">
                {correctMarked.length ? (
                    correctMarked.map((question) => (
                        <div className="border-2 border-[#4D6B73] rounded-md m-[25px]  p-3">
                            <p className="font-medium text-3xl m-2">
            ...
            <h2 className="text-3xl text-[#D64045] underline">
                Incorrectly Marked Questions
            </h2>

            <div className="incorrect ">
                {incorrectMarked ? (
                    incorrectMarked.map((question) => (
                        <div className="border-2 border-[#4D6B73] rounded-md m-[25px]  p-3">
                            <p className="font-medium text-3xl m-2">
                                {decodeHtml(question.question)}
                            </p>
            ...
            {score >= 1 && (
                <TimeJoke initial={initialTime} final={remainTime}></TimeJoke>
            )}
            <button
                className="bg-[#4D6B73] text-[#FFFEF2] p-2 rounded-md font-semibold text-xl border-2 border-black m-2"
                onClick={() => navigate('/')}
            >
                Retake Test
            </button>
